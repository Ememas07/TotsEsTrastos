...
public class Ud5MiniPracticaOrdenacioAmbExtresMarcMas {
    ...
    public static void bubbleSort(int taula[], int debugMode) {
        ...
        for (int i = 0; i < taula.length - 1; i++) { //recorr tot l'array menys 1
            for (int j = 0; j < taula.length - i - 1; j++) { //recorr tot l'array menys 1 menys els que ja tenim ordenats
                ...
                if (taula[j] > taula[j + 1]) { //si no estan ordenats, les gir
                    intercanvis++;
                    girarNombres(j, j + 1, taula, debugMode);
                }
            }
        }
    ...
    public static void girarNombres(int n1, int n2, int[] taula, int debugMode) {
        int temp = 0;
        temp = taula[n1];
        taula[n1] = taula[n2];
        taula[n2] = temp;
