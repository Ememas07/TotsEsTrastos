 ...
 * Nota: 1
 */
public class Ud1a4ProvaPractica2MOD_V2MarcMas {
    ...
    public static void tipusButaca(int butaca, int butacaTotal, int butacaNormal, int butacaVIP) { // MODIFICAT: faltaven els parametres de butacaNormal i butacaVIP
        switch (butaca) {
            ...
            case 2 -> {
                System.out.println("Has triat una butaca VIP");
                butacaVIP--;
                butacaTotal--;
            }
            default -> {
    ...
    public static float preuEntrada(int diaSetmana, int butaca, int butacaTotal, float descompte) {
        ...
        if (butacaTotal > 0) { //while canviat per if, explicat més abaix, i signe girat
            ...
            preuEntrada = (float) preuEntrada;
            preuEntrada *= 1 - descompte; //MODIFICAT: falta el ;
            return preuEntrada;
        } else { // Canviat un while per un if
    ...
    public static void menu() { //MODIFICAT: afegit public
        ...
        int anyNaixement = 0;
        while (anyNaixement < 1900 || anyNaixement > any) {
        ...
        final float IVA = 0.9f; // MODIFICAT: falta el "f" de float
        System.out.println("Premi 1 per confirmar la compra");
        confirmar = sc.nextInt();
        if (confirmar == 1) {
            contadorTaquilla += preuEntrada;
            contadorBar += totalBar;
            // int contadorBar; // ORIGINAL
            // int contadorGlobal = contadorTaquilla + contadorBar // ORIGINAL
            contadorGlobal = contadorTaquilla + contadorBar; // MODIFICAT: faltava el ";"
            ...
            System.out.println("Taquilla: " + contadorTaquilla);
            System.out.println("Bar: " + contadorBar);
            System.out.println("Total: " + contadorGlobal);
        }
    ...
    public static void main(String[] args) {
        ...
        int confirm2 = 0;
        while (confirmar != 1) {
            System.out.println("Introdueix un nombre negatiu per sortir");
            confirm2 = sc.nextInt();
            if (confirm2 < 0) {
                System.out.println("Segur que vol sortir? introdueix 1 per sortir");
                confirmar = sc.nextInt();
            }
            if (confirmar != 1) {
                menu();
