public class ExercisiArrays07 {
    // ) Crea dos arrays de números con una posición pasado por teclado. Uno de
    // ellos estará rellenado con números aleatorios y el otro apuntara al array
    // anterior, después crea un nuevo array con el primer array (usa de nuevo new
    // con el primer array) con el mismo tamaño que se ha pasado por teclado,
    // rellenalo de nuevo con números aleatorios. Después, crea un método que tenga
    // como parámetros, los dos arrays y devuelva uno nuevo con la multiplicación de
    // la posición 0 del array1 con el del array2 y así sucesivamente. Por último,
    // muestra el contenido de cada array. Llama al final al recolector de basura.

}
